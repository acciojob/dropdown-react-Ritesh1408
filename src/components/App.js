...
function App() 
{
    ...
    const handleCityChange = (e) => {
        ...
        setSelectCity(newCityIndex);
        setSelectLandmark(0);
    };

    const handleLandmarkChange = (e) => {
        const newLandmarkIndex = Number(e.target.value);
        setSelectLandmark(newLandmarkIndex);
    };

    const currentState = states[selectstate];
    const currentCity = currentState.city[selectcity];
    const currentLandmark = currentCity.landmarks[selectlandmark];
	...
	>
		...
		>
			...
			<select id="state" value={selectstate} onChange={handleStateChange}>
			...
			<select id="landmark" value={selectlandmark} onChange={handleLandmarkChange}>
